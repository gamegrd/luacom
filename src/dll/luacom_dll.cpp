...
extern "C" int __declspec(dllexport) luacom_openlib(lua_State* L) {
  luacom_open(L);
  return 0;
}


extern "C" int __declspec(dllexport) luaopen_luacom(lua_State* L) {
           ...
           )
{
  ...
  if(ul_reason_for_call == DLL_PROCESS_ATTACH) {
    auto x = OleInitialize(NULL);
    //printf("OleInitialize: %p", x);
    factoryCache_Init();
